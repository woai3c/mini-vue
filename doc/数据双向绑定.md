...
obj.a = 2 
```
像上面代码中的两个操作读取和赋值 就是在访问obj.a的getter和setter<br>
...
```
getter和setter都是一个函数 在我们读取他们的时候可以往里添加一些操作 在访问的时候也会执行这些操作 例如
```
get: function() {
  // 每次访问obj.a时都会执行这段代码
...
只要在读取值时收集依赖 在赋值时触发依赖 就可以实现数据变更重而实现DOM重新渲染

说到这可能还不是很明白 可以看一下这段代码 复制放到HTML文件里自己运行一下
